...
async def parseProductPage(url: str) -> dict:
    ...
    if response.status_code == 200:
        ...
        selector_values = html.xpath(
            '//select[@id="select_ware_size"]/option/@value')
        ...
        dump['description'] = ''.join(html.xpath(
            '//div[@class="wrapper row2"]/div[@id="container"]/div/p/text()'))
        dump['available'] = html.xpath(
            '//div[@class="three_fifth first"]//span/text()')[0]
        return (dump)
    ...
    if category_links:
        ...
        for category in category_links:
            ...
            if products_links:
                ...
                print(
                    f"Ошибка парсинга страницы категории или страница пуста: {category}")
